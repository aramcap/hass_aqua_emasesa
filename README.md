...
Hay una batería de tests unitarios del parseo de las respuestas de EMASESA, que es justo la parte que se rompe cuando el portal cambia algo (ver los puntos frágiles de arriba). No arrancan Home Assistant ni tocan la red: solo necesitan `pytest` y `aiohttp`, y tardan menos de un segundo.
...
Cubren `_parse_chart_response` (emparejado `ticks`/`data`, guiones escapados por PrimeFaces, listas descuadradas, y distinguir un rango sin datos de una sesión caducada o de una pantalla de verificación SMS), la conversión de etiquetas a fecha y a franja horaria (incluido el cruce de año de las cargas masivas de 90 días), la lectura del `maxDate` y del `ViewState`, y que el `statistic_id` generado para la estadística externa siempre lo acepte el recorder.

Las respuestas de ejemplo están en `tests/fixtures/` y reproducen el formato real del portal. Si EMASESA cambia el frontend, lo primero es recapturar una respuesta real con las DevTools, actualizar la fixture correspondiente y ver qué tests se caen.
