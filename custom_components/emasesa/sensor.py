...
class EmasesaLatestDaySensor(EmasesaBaseEntity):
    ...
    Se declara como `total` con `last_reset`, no como `measurement`:
    Home Assistant no admite la clase de estado `measurement` junto a la
    clase de dispositivo `water` (solo acepta `total`,
    `total_increasing` o ninguna), y de hecho este valor no es una
    medida instantánea, sino el total consumido a lo largo de un día
    completo. Poniendo `last_reset` en las 00:00 (hora de Sevilla) del
    día al que corresponde la lectura, el recorder trata cada día como
    un periodo de acumulación distinto y no arrastra el del día
    anterior.

    Para el Panel de Energía sigue siendo preferible la estadística
    externa horaria (ver `statistics.py`): tiene granularidad horaria y
    coloca cada consumo en su hora real, no en el momento en que EMASESA
    lo publica.
    """
    ...
    def last_reset(self) -> datetime | None:
        """Inicio (hora de Sevilla) del día al que corresponde el valor."""
        data = self.coordinator.data or {}
        latest = data.get("latest")
        if not latest:
            return None
        dia: date = latest["date"]
        return datetime(dia.year, dia.month, dia.day, tzinfo=ZONA_EMASESA)
